...
import { Badge, Button, Card, SectionHeader, SeamDivider } from "@/components/ui";
...
export default function SquadsClient({ teams, players, settings }: any) {
  ...
  return (
    <div>
      <SectionHeader eyebrow="Post-Auction" title="Final Team Squads" action={<Button variant="ghost" onClick={exportAllCSV}>Export All (CSV)</Button>} />
      <SeamDivider />
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-5">
        {teams.map((t: any) => {
          const squad = squadFor(t.id);
          return (
            <Card key={t.id} className="p-4 cursor-pointer" onClick={() => setPreviewTeam(t)}>
              <div className="flex items-center justify-between mb-3">
                <div className="text-sm font-bold font-display">{t.name}</div>
                <Badge tone="gold">{squad.length}/{settings?.max_squad_size ?? 14}</Badge>
              </div>
              <div className="text-[11px] mb-2 text-mutedDim">{t.owner_name || "No owner assigned"}</div>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {squad.slice(0, 5).map((p: any) => <div key={p.id} className="text-xs text-muted">{p.full_name} <span className="text-mutedDim">· {p.playing_role}</span></div>)}
                {squad.length > 5 && <div className="text-[11px] text-mutedDim">+{squad.length - 5} more…</div>}
                {squad.length === 0 && <div className="text-xs text-mutedDim">No players purchased yet.</div>}
              </div>
            </Card>
          );
        })}
      ...
      {previewTeam && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/75 backdrop-blur-sm" onClick={() => setPreviewTeam(null)}>
          <div className="w-full max-w-3xl max-h-[88vh] overflow-y-auto rounded-2xl border border-lineBright bg-bgPanel" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 flex items-center justify-between px-5 py-4 border-b border-line bg-bgPanel z-10">
              <h3 className="text-lg font-bold font-display">{previewTeam.name}</h3>
              <button onClick={() => setPreviewTeam(null)} className="w-8 h-8 rounded-full flex items-center justify-center bg-bgCard text-muted">×</button>
            </div>
            <div className="p-5">
              <div className="text-xs mb-4 text-mutedDim">Owner: {previewTeam.owner_name || "—"}</div>
              <div className="space-y-2 mb-5">
                {squadFor(previewTeam.id).map((p: any, i: number) => (
                  <div key={p.id} className="flex items-center justify-between py-2 border-b last:border-0 border-line">
                    <div><span className="text-sm font-semibold">{i + 1}. {p.full_name}</span><span className="text-[11px] ml-2 text-mutedDim">{p.playing_role} · {p.category}</span></div>
                    <span className="text-xs font-mono text-goldBright">{p.sold_points} pts</span>
                  </div>
                ))}
                {squadFor(previewTeam.id).length === 0 && <div className="text-sm text-mutedDim">No players purchased yet.</div>}
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Button variant="ghost" size="sm" onClick={() => printSquad(previewTeam)}>PDF / Print Squad</Button>
                <Button variant="orange" size="sm" onClick={() => downloadSquadImage(previewTeam)}>Download Squad Card (PNG)</Button>
              </div>
            </div>
          </div>
        </div>

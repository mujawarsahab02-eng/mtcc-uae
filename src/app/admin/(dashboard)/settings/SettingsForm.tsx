  ...
  return (
    <div>
      ...
      <fieldset disabled={!canEdit} style={{ opacity: canEdit ? 1 : 0.6 }}>
        ...
        <FormSection title="About & Mission">
          <p className="text-[11px] text-mutedDim mb-3">Shown on the homepage — tell people what MTCC UAE is about.</p>
          <Field label="About Text" hint="A paragraph or two introducing the tournament"><textarea value={draft.about_text || ""} onChange={set("about_text")} rows={4} /></Field>
          <Field label="Vision Statement"><textarea value={draft.vision_text || ""} onChange={set("vision_text")} rows={2} /></Field>
          <Field label="Mission Points" hint="One point per line — shown as a bulleted list"><textarea value={draft.mission_points || ""} onChange={set("mission_points")} rows={4} /></Field>
        </FormSection>
        ...
        <FormSection title="Competition Rules">
          ...
          <Field label="General Tournament Rules" hint="One rule per line — shown as a numbered list on the public Rules page"><textarea value={draft.general_rules || ""} onChange={set("general_rules")} rows={5} /></Field>
          <Field label="Match Conditions" hint="Powerplay, no-ball, wide-ball, LBW/byes — whatever differs from standard play"><textarea value={draft.match_conditions_rules || ""} onChange={set("match_conditions_rules")} rows={5} /></Field>
          <Field label="Substitution Rules"><textarea value={draft.substitution_rules || ""} onChange={set("substitution_rules")} rows={3} /></Field>
          <Field label="Super Over Rules"><textarea value={draft.super_over_rules || ""} onChange={set("super_over_rules")} rows={3} /></Field>
        </FormSection>

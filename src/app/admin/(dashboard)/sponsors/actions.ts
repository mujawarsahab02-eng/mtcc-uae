...
export async function addSponsor(name: string, logoPath: string | null, websiteUrl: string | null, isPoweredBy: boolean): Promise<any> {
  ...
  const { error } = await supabase.from("sponsors").insert({ name: name.trim(), logo_path: logoPath, website_url: websiteUrl, is_powered_by: isPoweredBy, sort_order: count ?? 0 });
